  ...
  return (
    <div className='pt-20 2xl:pt-40'>
      <div className="flex items-center justify-center min-h-[88vh]">
        <form onSubmit={handleSubmit} className='form flex flex-col gap-5 min-w-[50%] justify-around shadow-lg rounded-[10px] bg-[#1a1a1a] py-4 px-2 2xl:gap-10'>
        <p className="text-3xl font-bold text-cyan-300 2xl:text-6xl">Login</p>            
            {/* USERNAME */}
            ...
            <p className="2xl:text-4xl">Do not have an account? <Link to={"/auth/register"}  className="text-cyan-300 font-bold">Register</Link></p>

            <Button loading={loading} buttonText="Login"/>
            {error && error? <div className='p-3 border-l-4 border-red-500 bg-red-200 text-red-700 font-semibold rounded-lg shadow-sm 2xl:text-4xl 2xl:py-6 2xl:border-l-8'>{error}!</div>:""}
            {success && success? <div className='p-3 border-l-4 border-green-500 bg-green-200 text-green-700 font-semibold rounded-lg shadow-sm 2xl:text-4xl 2xl:py-6 2xl:border-l-8'>{success}!. Redirecting...</div>:""}
        </form>
      </div>
    </div>
  );
